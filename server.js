...
app.get("/api/reviews/:recipeId", (req, res) => {
  const recipeId = req.params.recipeId;
  const query = "SELECT ReviewID, Stars, Comments, UserID FROM Review WHERE RecipeID = ?";
  connection.query(query, [recipeId], (error, results) => {
    if (error) {
...
app.post("/api/reviews", (req, res) => {
  const { recipeId, userId, comments, stars } = req.body;
  if (!recipeId || !userId || !comments) {
    return res.status(400).json({ error: "Missing recipeId ,userId or comments" });
  }
  
  const query = stars 
  ? "INSERT INTO Review (RecipeID, UserID, Comments, Stars) VALUES (?, ?, ?, ?)"
  : "INSERT INTO Review (RecipeID, UserID, Comments) VALUES (?, ?, ?)";
  
  const params = stars ? [recipeId, userId, comments, stars] : [recipeId, userId, comments]; 
