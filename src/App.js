...
function HomePage() {
  ...
  const handleSelectRecipe = async (recipeId, titleOverride = null) => {
    try {
      ...
      const imageUrl = imageSearchOn
        ? await getRecipeImage(imageTitle)
  ...
  const fetchRandomRecipe = async () => {
    ...
    try {
      ...
      const imageUrl = imageSearchOn
        ? await getRecipeImage(recipeData.RecipeTitle)
        : "https://www.seriouseats.com/thmb/QJZXQHDXBfTiUSKstQ1uskJc31g=/1500x0/filters:no_upscale():max_bytes(150000):strip_icc()/SEA-best-grilled-hot-dogs-recipe-hero-02-9d245c0d43874a3da13a7228682b0dce.jpg";

      const ingResponse = await fetch(
        `http://localhost:3001/api/ingredients/${recipeData.RecipeID}`
      );
